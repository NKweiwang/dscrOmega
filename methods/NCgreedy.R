...
#' @param l an n-vector
NCest_f = function(Y,l,ash.maxit=1000){  
  betahat=(sum(l^2))^(-1)*(t(l)%*%Y) 
  betahat=as.vector(betahat)
  # sigmae2 = mean(Y^2)
  sigmae2 = apply(Y^2,2,mean)
...
NCest_l = function(Y,f,ash.maxit=1000){
  betahat=(sum(f^2))^(-1)*(t(f)%*%t(Y)) 
  betahat=as.vector(betahat)
  # sigmae2 = mean(Y^2)
  sigmae2 = apply(Y^2,1,mean)
